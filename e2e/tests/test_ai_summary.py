...
class TestAISummary:
    ...
    def test_ai_summary_streaming_response(self, search_page: SearchPage):
        """Intercept and validate the streaming response from the AI backend."""
        query = "What is air quality?"
        print(f"\nTesting streaming response for query: {query}")
        
        with search_page.page.expect_response(lambda r: "/_da/chat/send-message" in r.url) as response_info:
            search_page.search(query)
        ...
        streamed = StreamedResponse.from_jsonl_body(body)
        
        assert streamed.stopped is True
        assert len(streamed.get_message()) > 0
